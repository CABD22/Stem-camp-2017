...
Public Sub SyncMailToAsana(ByVal mail As Outlook.MailItem)
    ...
    If Len(Trim$(FinancialEmailConfig.WORKER_ENDPOINT)) > 0 Then
        PostViaWorker mail, primaryRecipient, mail.Subject
    Else
        PostDirectlyToAsana primaryRecipient, mail.Subject
    End If
End Sub

Private Sub PostViaWorker(ByVal mail As Outlook.MailItem, _
    ...
    http.Send BuildWorkerPayload(mail, recipient, subjectLine)
...
Private Function BuildWorkerPayload(ByVal mail As Outlook.MailItem, _
                                    ByVal recipient As String, _
                                    ByVal subjectLine As String) As String
    Dim recipientName As String

    If mail.Recipients.Count > 0 Then
        recipientName = Trim$(mail.Recipients(1).Name)
    End If

    If Len(recipientName) = 0 Then
        recipientName = StripAddress(mail.To)
    End If
    ...
    BuildWorkerPayload = "{" & _
                         q & "email" & q & ":" & q & JsonEscape(recipient) & q & "," & _
                         q & "name" & q & ":" & q & JsonEscape(recipientName) & q & "," & _
                         q & "subject" & q & ":" & q & JsonEscape(subjectLine) & q & "," & _
                         q & "date_sent" & q & ":" & q & _
                         JsonEscape(Format$(Now, "yyyy-mm-dd\THH:NN:SS\Z")) & q & "}"
End Function
